...
{
    class CalculateAdaptiveAutonomousRecursiveMovingAverage : AnalyzerBase, IMarketAnalyzer
    {
        public AnalyzerResults Analyze(IEnumerable<TickerData> tickers)
        {
            return base.ProceedThroughAnalyzing(tickers, (stockData) =>
            {
                return stockData.CalculateAdaptiveAutonomousRecursiveMovingAverage();
            });
        }
    }
}
